...
import {
  ...
  export class OtpSellerService {
    private readonly logger = new Logger(OtpSellerService.name);

    constructor(
      private readonly configService: ConfigService,
      @Inject('JwtForgotService') private readonly jwtForgotService: JwtService,
      @InjectRepository(OtpSeller) private readonly otpRepository: Repository<OtpSeller>,
      @InjectRepository(Seller) private readonly userRepository: Repository<Seller>,
    ) {}
  
    async generateOtp(email: string): Promise<OtpSeller> {
      try {
    ...
    async validateOtpVerification(
      ...
      const queryBuilder = this.otpRepository
        ...
        .innerJoin(Seller, 'seller', 'otp.sellerId = seller.id')
    ...
    async verifyOtpForgot(otpCode: string, email: string): Promise<string> {
      ...
      const payload = { id: otp.sellerId, email: email, role: 'seller' };
