import { Body, Controller, HttpStatus, Post, Put, Req, UnauthorizedException, UnprocessableEntityException, UseGuards } from "@nestjs/common";
...
//   }
  ...
  async updateSellerPartial(
    ...
    try {
      const updatedSeller = await this.userService.updateSellerPartial(req.user.id, updateSellerPartialDto);
      return new ResponseWrapper(HttpStatus.OK, 'Seller update successful');
    } catch (error) {
      if (error instanceof UnprocessableEntityException) {
        return new ResponseWrapper(HttpStatus.UNPROCESSABLE_ENTITY, error.message);
      } else if (error instanceof UnauthorizedException) {
        return new ResponseWrapper(HttpStatus.UNAUTHORIZED, error.message);
      } else {
        return new ResponseWrapper(HttpStatus.INTERNAL_SERVER_ERROR, 'Failed to update seller');
      }
    }
  }
